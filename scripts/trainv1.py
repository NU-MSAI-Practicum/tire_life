import os
import torch
import pandas as pd
import numpy as np
from env import TruckFleetEnv
# from dqn_agent import DQNAgent
from dqn_v2 import DQNAgent

from datetime import datetime
from copy import deepcopy
import matplotlib.pyplot as plt


def train(logs_folder):

    # Training loop
    num_episodes = 1000
    epsilon_start = 0.5
    epsilon_end = 0.001
    epsilon_decay = 10

    env = TruckFleetEnv(mean=0.5, std=0.1)
    agent = DQNAgent(state_dim=env.num_trucks * env.num_tires_per_truck, action_dims=[3, env.num_trucks, env.num_tires_per_truck, env.num_tires_per_truck])

    # To store the action logs for all episodes
    all_action_logs = []
    total_rewards = []

    for episode in range(num_episodes):
        initial_state = deepcopy(env.reset())
        state = initial_state.flatten()
        total_reward = 0

        for t in range(100):  # Limit number of steps per episode
            epsilon = epsilon_end + (epsilon_start - epsilon_end) * np.exp(-1. * episode / epsilon_decay)
            action = agent.select_action(state, epsilon)
            next_state, reward, done, _ = env.step(action)
            next_state = next_state.flatten()

            with torch.no_grad():
                current_q_value = agent.policy_net(torch.FloatTensor(state).unsqueeze(0)).view(-1, np.prod(agent.action_dims))
                action_index = np.ravel_multi_index(action, agent.action_dims)
                current_q_value = current_q_value[:, action_index].squeeze()
                max_next_q_value = agent.target_net(torch.FloatTensor(next_state).unsqueeze(0)).view(-1, np.prod(agent.action_dims)).max(1)[0].item()
                expected_q_value = reward + agent.gamma * max_next_q_value
                td_error = abs(expected_q_value - current_q_value.item())

            agent.memory.push(state, action, next_state, reward, td_error)
            state = next_state
            total_reward += reward

            agent.optimize_model()

            if done:
                print(f"Episode {episode} finished after {t+1} timesteps with total reward {total_reward}")
                break

        if episode % 10 == 0:
            agent.update_target_net()

        print(f"Episode {episode} - Total reward: {total_reward}")
        total_rewards.append(total_reward)
        action_logs = env.get_action_log()
        final_state = deepcopy(env.state)

        # Log details for this episode
        episode_log = {
            'Episode': episode,
            'Initial State': initial_state,
            'Final State': final_state,
            'Total Reward': total_reward,
            'Action Logs': action_logs,
            'Number of Actions': [len(log) for log in action_logs]
        }
        all_action_logs.append(episode_log)

        # Save details for this episode to an Excel file
        episode_df = pd.DataFrame({
            'Truck': [f'Truck {i}' for i in range(env.num_trucks)],
            'Initial State': [initial_state[i] for i in range(env.num_trucks)],
            'Final State': [final_state[i] for i in range(env.num_trucks)],
            'Number of Actions': [len(action_logs[i]) for i in range(env.num_trucks)],
            'Actions': [str(action_logs[i]) for i in range(env.num_trucks)]  # Ensure actions are converted to string for Excel
        })
        episode_df.to_excel(os.path.join(logs_folder, f'episode_{episode}_log.xlsx'), index=False)

    # Save the trained model
    torch.save(agent.policy_net.state_dict(), 'models/dqn_truck_fleet.pth')
    print("Training completed and model saved.")

    # # Plot total rewards
    # plt.plot(total_rewards, label='Total Reward')
    # plt.xlabel('Episode')
    # plt.ylabel('Total Reward')
    # plt.title('Total Reward per Episode')
    # plt.legend()
    # plt.show()

    # # Moving average
    # window = 50
    # moving_avg = np.convolve(total_rewards, np.ones(window)/window, mode='valid')
    # plt.plot(moving_avg, label='Moving Average Reward')
    # plt.xlabel('Episode')
    # plt.ylabel('Moving Average Reward')
    # plt.title('Moving Average Reward per Episode')
    # plt.legend()
    # plt.show()

    # # Evaluate the agent
    # average_eval_reward = evaluate_agent(env, agent)
    # print(f'Average evaluation reward over 100 episodes: {average_eval_reward}')

    # # Benchmark the agent against a random policy
    # average_random_reward = benchmark_random_policy(env)
    # print(f'Average random policy reward over 100 episodes: {average_random_reward}')



def evaluate_agent(env, agent, eval_episodes=100):
    eval_rewards = []

    for _ in range(eval_episodes):
        state = env.reset()
        state = state.flatten()
        total_reward = 0
        done = False

        while not done:
            action = agent.select_action(state, epsilon=0)  # Deterministic policy
            next_state, reward, done, _ = env.step(action)
            next_state = next_state.flatten()
            state = next_state
            total_reward += reward

        eval_rewards.append(total_reward)

    average_eval_reward = np.mean(eval_rewards)
    return average_eval_reward


def benchmark_random_policy(env, eval_episodes=100):
    random_rewards = []

    for _ in range(eval_episodes):
        state = env.reset()
        state = state.flatten()
        total_reward = 0
        done = False

        while not done:
            action = env.action_space.sample()  # Random action
            next_state, reward, done, _ = env.step(action)
            next_state = next_state.flatten()
            state = next_state
            total_reward += reward

        random_rewards.append(total_reward)

    average_random_reward = np.mean(random_rewards)
    return average_random_reward

def train_old(logs_folder):

    # Training loop
    num_episodes = 1000
    epsilon_start = 0.5
    epsilon_end = 0.001
    epsilon_decay = 10

    env = TruckFleetEnv(mean=0.5, std=0.1)
    agent = DQNAgent(state_dim=env.num_trucks * env.num_tires_per_truck, action_dims=[3, env.num_trucks, env.num_tires_per_truck, env.num_tires_per_truck])

    # To store the action logs for all episodes
    all_action_logs = []

    for episode in range(num_episodes):
        initial_state = deepcopy(env.reset())
        state = initial_state.flatten()
        total_reward = 0

        for t in range(100):  # Limit number of steps per episode
            epsilon = epsilon_end + (epsilon_start - epsilon_end) * np.exp(-1. * episode / epsilon_decay)
            action = agent.select_action(state, epsilon)
            next_state, reward, done, _ = env.step(action)
            next_state = next_state.flatten()

            agent.memory.push(state, action, next_state, reward)
            state = next_state
            total_reward += reward

            agent.optimize_model()

            if done:
                print(f"Episode {episode} finished after {t+1} timesteps with total reward {total_reward}")
                break

        if episode % 10 == 0:
            agent.update_target_net()

        print(f"Episode {episode} - Total reward: {total_reward}")
        action_logs = env.get_action_log()
        final_state = deepcopy(env.state)

        # Log details for this episode
        episode_log = {
            'Episode': episode,
            'Initial State': initial_state,
            'Final State': final_state,
            'Total Reward': total_reward,
            'Action Logs': action_logs,
            'Number of Actions': [len(log) for log in action_logs]
        }
        all_action_logs.append(episode_log)

        # Save details for this episode to an Excel file
        episode_df = pd.DataFrame({
            'Truck': [f'Truck {i}' for i in range(env.num_trucks)],
            'Initial State': [initial_state[i] for i in range(env.num_trucks)],
            'Final State': [final_state[i] for i in range(env.num_trucks)],
            'Number of Actions': [len(action_logs[i]) for i in range(env.num_trucks)],
            'Actions': [action_logs[i] for i in range(env.num_trucks)]
        })
        episode_df.to_excel(os.path.join(logs_folder, f'episode_{episode}_log.xlsx'), index=False)

    # Save the trained model
    torch.save(agent.policy_net.state_dict(), 'models/dqn_truck_fleet.pth')
    print("Training completed and model saved.")